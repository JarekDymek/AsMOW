...
context.localStorage = { getItem: key => memory.get(key), setItem: (key, value) => memory.set(key, value) };
context.WEEKLY_SETTINGS_KEY = 'settings';
context.WEEKLY_PLAN_KEY = 'plan';
context.document = { getElementById: () => null };
context.loadInternatScheduleIndex = () => [base, correction, dayOff];
context.renderWeeklyPlan = () => {};
context.setWeeklyStatus = () => {};
context.rebuildWeeklyPlanFromMail('Dymek');
const plan = JSON.parse(memory.get('plan'));
assert.equal(plan.educator, 'Dymek');
assert.equal(plan.weeks[0].summary.totalHours, 0);
assert.ok(plan.weeks[0].days.every(day => day.shifts.length === 0));
context.syncCurrentInfoMail = async () => ({ ok: true });
context.fetch = () => { throw new Error('Legacy Apps Script must not be contacted'); };
await context.fetchWeeklyPlan({ rescan: true });
console.log('OK: weekly view uses mail sync and does not resurrect a removed shift or call Apps Script.');
